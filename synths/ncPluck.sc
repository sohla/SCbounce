(
{
	...
	pitch = (36 + 24.rand);
	env = Decay2.ar(strike, 0.1, 0.02, BrownNoise.ar(3).tanh);


	sig = Pan2.ar(
			// array of 3 strings per note
			Mix.ar(Array.fill(5, { arg i;
				// detune strings, calculate delay time :
				detune = #[-0.06, 0, 0.04,0.21,-0.23].at(i);
			...
			noise = LFNoise2.ar((pitch+48).midicps, env); // 3000 Hz was chosen by ear..
				CombL.ar(noise,		// used as a string resonator
					...
					2,
				1,
				SinOsc.ar(pitch.midicps*0.5,0, Decay2.ar(strike,0.01,pitch.linexp(36,100,1,0.1)*0.2));
