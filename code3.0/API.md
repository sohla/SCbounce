...
| `/airkit/seatTrim` | `seat(1–5) linearGain(0–2.0) [fadeSec=0.5]` | [COTF] per-seat static trim, multiplied with the boot room trim on that seat's monitor. Independent of voiceMute's gain and masterLevel's masterGain; never touches other seats. NOT persisted by M0 — bake tuned values into `~cotfSeatTrims` (main_cotf.scd) so restarts re-assert them. Baked default (2026-08-06): Room 3 seat 4 −5 dB (0.5623), everything else 1. |
