...
| `/airkit/seatTrim` | `seat(1–5) linearGain(0–2.0) [fadeSec=0.5]` | [COTF] per-seat static trim, multiplied with the boot room trim AND (2026-08-18) the loaded personality's per-patch trim (`~cotfPatchTrims`, main_cotf.scd — Room 3 loudness compensation now travels with the SOUND for super-seats, applied on the local `/airkit/personalityName` broadcast; Room 3 `~cotfSeatTrims` is flat) on that seat's monitor. Independent of voiceMute's gain and masterLevel's masterGain; never touches other seats. NOT persisted by M0 — bake tuned values into `~cotfSeatTrims` (chair-shaped) or `~cotfPatchTrims` (sound-shaped) so restarts re-assert them. Baked patch trims (2026-08-18, migrated from the old Room 3 seat trims + Ciaran's now-zeroed MOTU 12–16 offsets): BASSBUZZ +6.5 dB, JUPITERSHARP +2 dB, ALTOSYNTH +11 dB, PERCUSSION −5 dB, SOPRANOVOICE +5 dB (pulled from +16 on 2026-08-19 pending the overnight dynamics rework's by-ear check). Room 2 seat trims unchanged (seat 3 +5 dB). |
