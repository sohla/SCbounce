...
//------------------------------------------------------------
SynthDef(\samplerHB, {|bufnum=0, out=0, amp=0.5, rate=1, start=0, pan=0, freq=440, attack=0.01, decay=0.1, sustain=0.9, release=0.2, gate=1,cutoff=20000, rq=1, loop=1|
...
//------------------------------------------------------------
~init = ~init <> {
	var folder  = PathName("~/Downloads/yourDNASamples/bath/heather");
...
//------------------------------------------------------------
~next = {|d|
	var amps = [1,1,1,1] * 0.3;
  states.do({|o,i|
	    if(d.sensors.digiInEvent[i] == 1, {
      if(o == 1,{
      	states[i] = 0;
        //max 11
				synths.put(i, Synth(\samplerHB, [\bufnum, buffers[0], \rate, 1 + (i*0.4), \amp, amps[i]]));
      	// ["on",i].postln;
    	});
		},{
      if(o == 0,{
