...
SynthDef(\pullstretchMonoQ, {|out, amp = 1, buffer = 0, envbuf = -1, pch = 1, div=1, speed = 0.1, splay = 0.3,pan=0, gate=1|
	var len = BufDur.kr(buffer) / div;
	var lfo = LFSaw.kr( (1.0/len) * speed ,1).range(0.0,0.9);
	var sp = Splay.arFill(8,
		{ |i| Warp1.ar(1, buffer, lfo.linlin(0,1,0.05,0.95), pch * (0.midiratio) * [1,4]  ,splay, envbuf, 8, 0.1 * (i+1), 4)  },
			1,
			1,
			0
	) ;
	...
	Out.ar(out,Pan2.ar(mas[0],pan)* amp.lag(0.4) * env);
}).add;
//------------------------------------------------------------
~init = ~init <> {
