...
SynthDef(\pullstretchMonoQ, {|out, amp = 1, buffer = 0, envbuf = -1, pch = 1, div=1, speed = 0.01, splay = 0.3,pan=0, gate=1, delta=0.0, lag=0.05, ffo=11 rfo=1,pos=0|
  ...
  var afo = LFCub.ar(ffo,0,rfo).range(1.0 - rfo,2.0 - rfo);
	var pp = #[0,4,7,12].midiratio;
	var sp = Splay.arFill(4,
		{ |i| Warp1.ar(1, buffer, lfo.linlin(0,1,0.2,0.75), pch * (1 / ((i*delta)+1)) * [1] ,splay, envbuf, 8, 0.1 * (i+1), 4)  },
		// { |i| Warp1.ar(1, buffer, lfo.linlin(0,1,0.0,0.1), pch * pp[i],splay, envbuf, 8, 0.1 * (i+1), 4)  },
			1,
			1,
			0
	) ;
	...
	// Out.ar(out, ((0)!0 ++ sig));
	Out.ar(out, ((0)!2 ++ sig ++ ((0)!4) ++ (sig*0.3)));

}).add;
//------------------------------------------------------------
~init = ~init <> {
	var path = PathName("~/Downloads/nicSamples/3_Bites/Bits/1.wav");
...
~next = {|d|
  ...
  var ffo = m.gyroYFiltered.lincurve(-1.0,1,6,12,-3);
