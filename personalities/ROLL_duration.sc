...
Event.addEventType(\customEvent, {|e|
	...
	Pdef(m.ptn,
		Pbind(
			\type, \customEvent,
			\instrument, \stereoSampler,
			\note, Pseq([0,4,7,12,11,7,4,2], inf),
			\root, Pseq([0,4,-3].stutter(16 * 3), inf),
			\octave, Pseq([5].stutter(1), inf),
...
~next = {|d|
	...
	var dur = m.rrateMassFiltered.lincurve(0,1.0,0.4,0.125*0.25,-3);
	var move = m.accelMassFiltered.linlin(0,3,0,1);
	var amp = m.rrateMassFiltered.lincurve(0,1.0,-30,-14,-1);

	Pdef(m.ptn).set(\dur, dur);

	if(m.rrateMassFiltered > 0.07,{
		Pdef(m.ptn).set(\amp, amp.dbamp * 0.55);
	},{
	...
	if(move > 0.42, {
		if(TempoClock.beats > (lastTime + 0.75),{
			lastTime = TempoClock.beats;
			~playNote.(m.com.root,0, 3);
		});
	});
};
