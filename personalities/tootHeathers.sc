...
//------------------------------------------------------------
SynthDef(\samplerHT, {|bufnum=0, out=0, amp=1, rate=1, start=0, pan=0, freq=440, attack=0.01, decay=0.1, sustain=0.9, release=0.2, gate=1,cutoff=20000, rq=1, loop=1|
...
~init = ~init <> {
	...
	var folder  = PathName("~/Downloads/yourDNASamples/bath/balloon");
...
~next = {|d|
    ...
    var ndx = (buffers.size -1).rand;
      bl = true;
      synth = Synth(\samplerHT, [\bufnum, buffers[ndx], \gate, 1, \rate, ([4]).choose.midiratio, \amp, 0.5]);
    });
 },{
    if(bl == true,{
        bl = false;
