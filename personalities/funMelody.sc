...
~init = ~init <> {
	Pdef(m.ptn,
		Pbind(
			...
			\envSus, 0.0,
			\envRel,Pkey(\octave).squared * 0.03,
...
~next = {|d|
	...
	Pdef(m.ptn).set(\filtFreq, m.accelMassFiltered.linexp(0,1,180,14000));
	Pdef(m.ptn).set(\amp, m.accelMassFiltered.lincurve(0,0.3,0.11,0.045,-2));
