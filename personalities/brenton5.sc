...
//------------------------------------------------------------
SynthDef(\pullstretchMonoQB, {|out, amp = 1, buffer = 0, envbuf = -1, pch = 1.0, div=1, speed = 0.01, splay = 0.4 ,pan=0, ff=245|
	var pos;
	var len = BufDur.kr(buffer) / div;
	var lfo = LFSaw.kr( (1.0/len) * speed ,1,0.5,0.5);

	var sp = Splay.arFill(4,
		{ |i| Warp1.ar(1, buffer, lfo.linlin(0,1,0.35,0.77), pch,splay, envbuf, 4, 0.3, 4)  },
			1,
			1,
			0
	) ;
...
~next = {|d|
	...
	var speed= m.accelMassFiltered.lincurve(0.5,2.5,0.001,1.0,-2);
	var ff = m.accelMassFiltered.lincurve(0.0,2.5,245,2450,-1);
	var rate = m.accelMassFiltered.linlin(0,1,0.9,1.4);
	var pch = m.gyroYFiltered.linlin(-1,1,-12,50).asInteger ;
