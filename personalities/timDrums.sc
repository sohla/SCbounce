...
~next = {|d|
	...
	var dur = 0.2 * 2.pow(m.accelMassFiltered.linlin(0,3,0,3)).reciprocal / 1.5;
	...
	if(d.sensors.digiInEvent[0] == 1, {
		if( Pdef(pa).isPlaying.not,{
			Pdef(pa).resume(quant:0);
		});
	},{
		if( Pdef(pa).isPlaying,{
			Pdef(pa).pause();
		});
	});
