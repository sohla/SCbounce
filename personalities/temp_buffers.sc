...
//------------------------------------------------------------
SynthDef(\pullstretchStereo, {|out, amp = 1, bufnum = 0, envbuf = -1, pch = 1, div=1, speed = 0.5, splay = 0.1 ,pan=0, gate=1, ws=3|
	var len = BufDur.kr(bufnum) / div;
	var lfo = LFSaw.kr( (1.0/len) * speed,1);
	var sp = Splay.arFill(2,
		{ |i| Warp1.ar(2, bufnum, lfo.linlin(-1,1,0.0,0.65), pch * 2.pow(i),splay, envbuf, ws, 0.0 , 2)  },
			1,
			1,
			0
	) ;
...
~init = ~init <> {
	...
	var folder  = PathName("~/Downloads/melSamples/hello");
	...
	buffers = folder.entries.collect({ |path,i|
        ...
        var winenv = Env([0, 1, 0], [0.002, 0.5], [8, 1]);
        var z = Buffer.sendCollection(s, winenv.discretize, 1);
				"samples loaded".postln;
    		synth = Synth(\pullstretchStereo, [\bufnum, buffers[4],  \envbuf, z, \pch, 0.5]);
			});
		});
	});
};
