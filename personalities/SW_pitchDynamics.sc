...
SynthDef(\treeWind, { |out, frq=111, gate=0, amp = 0, note=0, rate = 4, mix = 0.5|
	...
	[
		Decay.ar(Impulse.ar(rate,0,0.1),2,0.05),
		PinkNoise.ar(0.01)
	],0.98);
	var sig =  DynKlank.ar(`[[48 + note.lag(0.01)].midicps, nil, [2, 1, 1, 1]], trig * env * follow);
...
~next = {|d|
	...
	var amp = m.rrateMassFiltered.lincurve(0.0,1.0,0.0,0.5);
	var notes = [0,2,4,5,7,9,11,12];
	var index = (d.sensors.gyroEvent.y/pi).linlin(-0.5,0.5,0.0,notes.size-1); 
	var mix = m.accelMassFiltered.lincurve(0.0,2.5,1.0,0.0,-2); 

	var rate = m.accelMassFiltered.lincurve(0,2.5,0.1,20,-2);
