...
SynthDef(\fuckYes, {
	...
	var source = SinOsc.ar(freq * Demand.kr(trig, 0, seq) * 0.5, 0,(0.9+amp).distort);
	var sig = Pluck.ar(source + (in * 0.1), Dust.ar(LFCub.ar(1/120,0,2.5).tanh.lag(0.3).linlin(-1,1,1,900)), freq.reciprocal, freq.reciprocal, 1,
        coef:0.5)!2 * 0.8;
	sig = sig.softclip.distort;
	sig = HPF.ar(sig, 30);
	sig = LPF.ar(sig, 9.9e4);
	// sig = DelayC.ar(sig,0.3,0.28);
	// sig = GVerb.ar(sig/3);
	LocalOut.ar(sig);
	sig = Mix.ar([sig, source]);
	sig = PitchShift.ar(sig,0.2, [0.252,0.125] * Demand.kr(trig, 0, seq) * 2) ;
	sig = BHiShelf.ar(sig, 9000, db:-4) * 0.9;
	sig = BLowShelf.ar(sig, 280, db:4) * 0.9;
